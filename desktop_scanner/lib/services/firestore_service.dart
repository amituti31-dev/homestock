...
class FirestoreService {
  ...
  String _shoppingListPath(String householdId, String itemId) =>
      '${FirebaseConfig.documentsPath}/households/$householdId/shoppingList/$itemId';

  /// Adds an item to the shared shopping list when it's just run out, so it
  /// shows up on the phone's shopping list screen — mirrors the mobile app's
  /// `addLowStockToShoppingList`, minus the model class desktop has no other
  /// use for. Skips if an unchecked auto-generated entry for the same
  /// inventory item is already there.
  Future<void> addToShoppingList(String householdId, InventoryItem item) async {
    try {
      if (await _hasUnclaimedShoppingListEntry(householdId, item.id!)) return;

      final listItemId = _autoId();
      await _commit([
        {
          'update': {
            'name': _shoppingListPath(householdId, listItemId),
            'fields': {
              'name': encodeString(item.name),
              'quantity': encodeDouble(1),
              'unit': encodeString(item.unit),
              'store': const {'nullValue': null},
              'category': encodeString(item.category.name),
              'checked': encodeBool(false),
              'autoGenerated': encodeBool(true),
              'inventoryItemId': encodeString(item.id),
            },
          },
          'updateTransforms': [_serverTimestamp('addedAt')],
          'currentDocument': {'exists': false},
        }
      ]);
    } catch (e) {
      throw Exception('הוספה לרשימת הקניות נכשלה: ${_bare(e)}');
    }
  }

  Future<bool> _hasUnclaimedShoppingListEntry(
      String householdId, String inventoryItemId) async {
    final response = await http.post(
      _uri('households/$householdId:runQuery'),
      headers: await _headers(),
      body: jsonEncode({
          ...
          'where': {
            'compositeFilter': {
              ...
              'filters': [
                {
                  'fieldFilter': {
                    'field': {'fieldPath': 'inventoryItemId'},
                    'op': 'EQUAL',
                    'value': {'stringValue': inventoryItemId},
                  }
                },
                {
                  'fieldFilter': {
                    'field': {'fieldPath': 'checked'},
                    'op': 'EQUAL',
    ...
    // Best-effort dedup: a failed check just risks a duplicate list entry,
    // not a lost one, so it shouldn't block the add.
    if (response.statusCode != 200) return false;
    final rows = jsonDecode(response.body) as List;
    return rows.any((row) => (row as Map<String, dynamic>).containsKey('document'));
  }
