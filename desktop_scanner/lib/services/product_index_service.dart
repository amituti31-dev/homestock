...
class ProductIndexService {
  ...
  /// Loads the bundled Shufersal seed (`assets/shufersal_seed.json`,
  /// generated by `tool/generate_shufersal_asset.dart`) and merges it into
  /// the index. Not a live download: some machines fail TLS verification
  /// against prices.shufersal.co.il (or GitHub, for `downloadSeed()`) for
  /// reasons outside this app's control, so a bundled snapshot trades
  /// freshness for working identically on every install. Barcodes from other
  /// chains (imported separately, e.g. via tool/import_price_file.dart) are
  /// kept — only barcodes in the bundled seed are added or updated.
  /// Refreshing the data means regenerating the asset and shipping a new
  /// release, not re-running this. Returns the total number of products in
  /// the index after the merge.
  Future<int> refresh() async {
    final json = await rootBundle.loadString('assets/shufersal_seed.json');
    ...
    if (products.isEmpty) {
      throw Exception('קובץ המחירים המצורף לא הכיל מוצרים');
    }
  ...
  /// `tool/generate_shufersal_asset.dart` to regenerate the bundled seed —
  /// not called at runtime, since `refresh()` reads the already-parsed
  /// asset instead of downloading and parsing XML live.
  ///
